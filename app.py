...
class EmployeeWindow(tk.Toplevel):
    def __init__(self, master):
        ...
        # Создайте фрейм для размещения кнопок
        ...
        self.delete_employee_button.grid(row=0, column=1, padx=5, pady=5)

        # Создаем кнопку для редактирования сотрудника
        self.edit_employee_button = tk.Button(button_frame, text="Редактировать сотрудника", command=self.edit_employee)
        self.edit_employee_button.grid(row=0, column=2, padx=5, pady=5)
    ...
    # Добавьте функцию для отображения результатов поиска
    ...
    # Измените метод search_employee
    def search_employee(self):
        search_query = '%' + self.search_entry.get() + '%'
        cursor.execute("SELECT * FROM employee WHERE full_name LIKE ?", (search_query,))
        data = cursor.fetchall()
...
class EditEmployeeWindow(tk.Toplevel):
    def __init__(self, parent, employee_id, employee_window):
        ...
        save_button.grid(row=10, columnspan=2, padx=10, pady=10)
    ...
    def save_employee(self):
        ...
        new_hire_date = self.hire_date_entry.get()
        ...
        cursor.execute("UPDATE employee SET full_name=?, gender=?, birth_date=?, education=?, position=?, profession=?, hire_date=?, salary=?, passport_data=?, address=? WHERE employee_id=?",
                       (new_full_name, new_gender, new_birth_date, new_education, new_position, new_profession, new_hire_date, new_salary, new_passport_data, new_address, self.employee_id))
        conn.commit()
...
class EditDepartmentWindow(tk.Toplevel):
    def __init__(self, parent, department_id, department_window):
        ...
        tk.Label(self, text="ID сотрудника:").grid(row=3, column=0, padx=10, pady=5)
        ...
        self.employee_id_entry.grid(row=3, column=1, padx=10, pady=5)
        self.employee_id_entry.insert(0, self.department_data[4])
    ...
    def save_department(self):
        ...
        new_employee_id = self.employee_id_entry.get()

        # Обновите все поля данных отдела в базе данных
        cursor.execute("UPDATE department SET name=?, head=?, contact_info=?, employee_id=? WHERE department_id=?",
                       (new_department_name, new_head, new_contact_info, new_employee_id, self.department_id))
        conn.commit()
